...
type stubCollector struct {
	...
	optional    bool
	collectFunc func(env *Env) (string, error)
	callCount   int
	mu          sync.Mutex
}
...
func (s *stubCollector) Collect(env *Env) (string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.callCount++
	if s.collectFunc != nil {
		return s.collectFunc(env)
	}
...
func TestManager_Get(t *testing.T) {
	...
	t.Run("cache miss collects fresh data", func(t *testing.T) {
		...
		// Act
		result, err := m.Get(ContentModel, &Env{})
	...
	t.Run("cache hit returns cached data without re-collecting", func(t *testing.T) {
		...
		result1, err1 := m.Get(ContentModel, &Env{})
		require.NoError(t, err1)

		// Act - second call should use cache
		result2, err2 := m.Get(ContentModel, &Env{})
	...
	t.Run("collector error is returned", func(t *testing.T) {
		...
		m := NewManager()
		stub := newStubCollector(ContentModel, 5*time.Second, false)
		stub.collectFunc = func(env *Env) (string, error) {
...
func TestManager_ClearTypeCache(t *testing.T) {
	...
	_, err1 := m.Get(ContentModel, &Env{})
	require.NoError(t, err1)
	_, err2 := m.Get(ContentTokenBar, &Env{})
	require.NoError(t, err2)
...
func TestGetAll_PanicRecovery(t *testing.T) {
	...
	panicking.collectFunc = func(env *Env) (string, error) {
...
func TestGetAll_TimeoutSkipsSlowCollector(t *testing.T) {
	...
	slow.collectFunc = func(env *Env) (string, error) {
...
func TestGetOptionalContent_PanicRecovery(t *testing.T) {
	...
	// Act — should NOT panic
	result := m.GetOptionalContent(&Env{})
...
func TestCollectWithTimeout_NormalCollector(t *testing.T) {
	...
	// Act
	value, ok := m.collectWithTimeout(ContentModel, &Env{})
...
func TestCollectWithTimeout_CustomCollectorTimeout(t *testing.T) {
	...
	blockCh := make(chan struct{})
	custom.collectFunc = func(env *Env) (string, error) {
	...
	value, ok := m.collectWithTimeout(ContentQuota, &Env{})
